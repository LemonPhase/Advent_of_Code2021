...
def search_data(data_array, index, is_most):
    ...
    #print("data_len=%d, data_array_len=%d, index=%d, expected_bit=%s" % (len(data_array[0]),len(data_array), index, bit))
    filtered_data = []
    for data in data_array:
        if data[index] == bit:
    ...
    #print("search for oxygen: index=%d, filter_array_len = %d" % (index, len(filtered_data)))
